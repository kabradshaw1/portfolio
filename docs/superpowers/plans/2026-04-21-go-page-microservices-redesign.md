# /go Page Microservices Redesign Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Redesign the `/go` page to showcase the decomposed microservices architecture as the featured view, preserve the original monolith content in a second tab, and pull the AI assistant into its own standalone section.

**Architecture:** Single-file rewrite of `frontend/src/app/go/page.tsx`. Convert from server component to client component (`"use client"`), add `useState` for tab toggle, write new Microservices tab content with Mermaid diagrams, move existing content to Original tab, relocate AI assistant section below tabs.

**Tech Stack:** Next.js, React (`useState`), TypeScript, `MermaidDiagram` component (existing)

**Spec:** `docs/superpowers/specs/2026-04-21-go-page-microservices-redesign.md`

---

### Task 1: Rewrite page.tsx with tabbed layout and new content

This is a single-file rewrite. The file is 369 lines and being reorganized, so a full Write is cleaner than incremental edits.
...
Replace the entire contents of `frontend/src/app/go/page.tsx` with:
...
Expected: 0 errors (warnings are OK — they're pre-existing).

- [ ] **Step 4: Visual check**

Open `http://localhost:3000/go` and verify:
- Microservices tab is active by default
- "Why Decompose?", tech stack pills, architecture diagram, saga sequence diagram, and "What Changed" cards all render
- Clicking "Original" tab shows old content (architecture, checkout flow, stress testing)
- AI Shopping Assistant appears below both tabs
- CTA buttons appear between tabs and AI section

- [ ] **Step 5: Commit**

```bash
git add frontend/src/app/go/page.tsx
git commit -m "feat(frontend): redesign /go page with microservices architecture tabs"
```

---

### Task 2: Push and verify CI
