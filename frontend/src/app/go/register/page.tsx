...
function GoRegisterPageInner() {
  ...
  const handleSubmit = useCallback(
    ...
    [email, password, name, register, router, searchParams],
  );
  ...
  return (
    <div className="mx-auto max-w-sm px-6 py-12">
      ...
      <p className="mt-6 text-center text-sm text-muted-foreground">
        ...
        <Link href={searchParams.get("next") ? `/go/login?next=${searchParams.get("next")}` : "/go/login"} className="underline hover:text-foreground">
          Sign in
        </Link>
      </p>
    </div>
