...
func TestTrendingAggregatorV2_HandleView(t *testing.T) {
	t.Parallel()

	now := time.Date(2026, 4, 22, 14, 0, 0, 0, time.UTC)
	clock := window.NewMockClock(now)
	s := store.NewMockStore()
	agg := NewTrendingAggregatorV2(time.Hour, 5*time.Minute, 5*time.Minute, clock, s)
...
func TestTrendingAggregatorV2_HandleCartAdd(t *testing.T) {
	t.Parallel()
...
func TestTrendingAggregatorV2_CombinedScoring(t *testing.T) {
	t.Parallel()
...
func TestTrendingAggregatorV2_FlushSendsScoresToStore(t *testing.T) {
	t.Parallel()
...
func TestTrendingAggregatorV2_DroppedEvent(t *testing.T) {
	t.Parallel()
