...
func TestSafePublishRecordsSuccessAndSwallowsErrors(t *testing.T) {
	...
	SafePublish(context.Background(), fakeProducer{}, "ecommerce.cart", "key", Event{Type: "cart.updated"})
	SafePublish(context.Background(), fakeProducer{err: errors.New("write failed")}, "ecommerce.cart", "key", Event{Type: "cart.updated"})
