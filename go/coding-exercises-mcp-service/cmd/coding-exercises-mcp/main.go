...
import (
	...
	"github.com/kabradshaw1/portfolio/go/coding-exercises-mcp-service/internal/mcpserver"
...
func main() {
	...
	cfg := config{
		dbPath:       getenv("CODING_EXERCISES_DB_PATH", defaultDBPath),
		materialPath: getenv("CODING_EXERCISES_MATERIAL_PATH", defaultMaterialPath),
	}

	if err := run(ctx, cfg, logger, runMCPServer); err != nil {
		logger.Fatalf("coding exercises MCP server failed: %v", err)
	}
}
...
func run(ctx context.Context, cfg config, logger *log.Logger, runServer serverRunner) error {
	...
	service := coding.New(db, cfg.materialPath)
	...
	logger.Printf("coding exercises MCP server running on stdio db_path=%s material_path=%s", cfg.dbPath, cfg.materialPath)
