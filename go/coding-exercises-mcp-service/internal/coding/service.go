...
func (s *Service) SubmitAnswer(ctx context.Context, questionID int64, reviewSummary string) (AnswerReview, error) {
	if questionID <= 0 {
		return AnswerReview{}, fmt.Errorf("question_id is required")
	}
	if reviewSummary == "" {
		return AnswerReview{}, fmt.Errorf("review_summary is required")
	}
	if s.sessionID == 0 {
...
func (s *Service) UpdateExpectedAnswer(ctx context.Context, questionID int64, expectedDesign string) error {
	if questionID <= 0 {
		return fmt.Errorf("question_id is required")
	}
	if expectedDesign == "" {
		return fmt.Errorf("expected_design is required")
	}
	return s.store.UpdateExpectedAnswer(ctx, questionID, expectedDesign)
}
