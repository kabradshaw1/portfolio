...
func TestCartRepository_CRUD(t *testing.T) {
	...
	repo := repository.NewCartRepository(infra.Pool, newBreaker())
	...
	if item.Quantity != 3 {
		t.Errorf("expected quantity=3 after first add, got %d", item.Quantity)
	}

	// Add same item again (upsert) with qty=2 — expect qty=5.
	...
	// GetByUser — verify JOIN returns product name.
	cartItems, err := repo.GetByUser(ctx, userID)
	if err != nil {
	...
	if cartItems[0].ProductName == "" {
		t.Error("expected non-empty ProductName from JOIN")
	}
	if cartItems[0].Quantity != 5 {
		t.Errorf("expected quantity=5 in GetByUser, got %d", cartItems[0].Quantity)
	}

	// UpdateQuantity.
	itemID := cartItems[0].ID
	if err := repo.UpdateQuantity(ctx, itemID, userID, 10); err != nil {
		t.Fatalf("UpdateQuantity: %v", err)
	}
	updated, err := repo.GetByUser(ctx, userID)
	if err != nil {
		t.Fatalf("GetByUser after update: %v", err)
	}
	if updated[0].Quantity != 10 {
		t.Errorf("expected quantity=10 after update, got %d", updated[0].Quantity)
	}

	// RemoveItem.
	if err := repo.RemoveItem(ctx, itemID, userID); err != nil {
		t.Fatalf("RemoveItem: %v", err)
	}

	// Verify cart is empty.
	empty, err := repo.GetByUser(ctx, userID)
	if err != nil {
		t.Fatalf("GetByUser after remove: %v", err)
	}
	if len(empty) != 0 {
		t.Errorf("expected empty cart, got %d items", len(empty))
	}
}
